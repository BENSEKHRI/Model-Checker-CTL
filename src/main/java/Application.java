...
public class Application {
    ...
    public static void menuSK2(KripkeStr k) {
        ...
        try (InputStream dot = Application.class.getResourceAsStream("KS.dot")) {
            MutableGraph g = new guru.nidi.graphviz.parse.Parser().read(dot);
            Graphviz.fromGraph(g).render(Format.PNG).toFile(new File("src/main/resources/KS.png"));
        } catch (IOException e) {
    ...
    public static void menuCTL1(KripkeStr k, Parser parser) {
        ...
        try {
            ...
            if (statesCheckCTL.size() == k.getStates().size()) {
                System.out.println("+-------------------------------------------+\n| \033[32mInfo : La SK vérifie cette formule CTL !\033[0m  |\n+-------------------------------------------+\n");
            }
